...
def health_route(category):
    def read(query: Annotated[HealthQuery, Query()], user=Depends(authenticate), service=Depends(get_health)):
...
for category in HEALTH_MEASUREMENTS:
    router.add_api_route("/api/health/" + category, health_route(category), methods=["GET"], response_model=HealthResponse)
...
def devices(request: Request, user=Depends(authenticate), service=Depends(get_health)):
    if request.query_params:
        raise APIError("INVALID_DEVICE_QUERY", "Device identity is controlled by the server; query parameters are not accepted.", 422)
