...
def log_metric_http_error(metric_name, error):
    ...
    if status in (403, 404):
        logger.warning(
            "%s unavailable: permission or device capability (HTTP %s)",
            metric_name,
            status,
        )
    else:
...
class ProviderHTTPClient:
    ...
    def request(
        ...
        data=None,
        request_type="get",
        suppress_http_error_log=False,
    ):
        cfg = self.settings
        if request_type not in {"get", "post"}:
            raise ValueError("Only GET and POST provider requests are supported")
        if cfg.health_api_provider == "google" and url.startswith(
            cfg.fitbit_api_base_url
        ):
        ...
        for attempt in range(cfg.request_max_retries + 1):
            ...
            try:
                ...
                if status == 429:
                    ...
                    if attempt >= min(
                        cfg.expired_token_max_retry, cfg.request_max_retries
                    ):
                        response.raise_for_status()
                    self.token_manager.refresh()
                elif status in {500, 502, 503, 504}:
                    if attempt >= min(
                        cfg.server_error_max_retry, cfg.request_max_retries
                    ):
                        if cfg.skip_request_on_server_error:
                            logger.warning(
                                "Skipping metric after repeated provider server failures (HTTP %s)",
                                status,
                            )
                            return None
                        response.raise_for_status()
                ...
                if attempt >= cfg.request_max_retries:
                    raise ProviderError(
                        "Provider network retry budget exhausted"
                    ) from None
