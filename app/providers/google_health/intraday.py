...
def map_intraday(
    ...
    for measurement in measurement_list:
        ...
        if not data_type:
            logger.warning(
                "Google mapping not available for intraday type: %s", measurement[0]
            )
            continue
            ...
            records.append(
                {
                    ...
                    "fields": {"value": int(numeric_value)},
                }
            )
            inserted_count += 1
        logger.info(
            "Recorded %s intraday for date %s (Google mode): %s points",
            measurement[1],
            date_str,
            inserted_count,
        )
