...
def map_hrv(data, start_date_str, end_date_str, device_name, local_timezone):
    records = []
    ...
    for data_point, ts in points:
        ...
        records.append({'measurement': 'HRV', 'time': ts, 'tags': {'Device': device_name}, 'fields': fields})
        inserted_count += 1
    if inserted_count:
        logger.info('Recorded HRV for date %s to %s (Google mode): %s points', start_date_str, end_date_str, inserted_count)
    else:
        logger.warning('No HRV records found for date %s to %s in Google mode', start_date_str, end_date_str)
    return [HealthPoint.from_record(record) for record in records]


def map_breathing(data, start_date_str, end_date_str, device_name, local_timezone):
    records = []
    ...
    for data_point, ts in points:
        ...
        if value is None:
            continue
        records.append({'measurement': 'BreathingRate', 'time': ts, 'tags': {'Device': device_name}, 'fields': {'value': float(value)}})
        inserted_count += 1
    if inserted_count:
        logger.info('Recorded BR for date %s to %s (Google mode): %s points', start_date_str, end_date_str, inserted_count)
    else:
        logger.warning('No Breathing Rate records found for date %s to %s in Google mode', start_date_str, end_date_str)
    return [HealthPoint.from_record(record) for record in records]


def map_temperature(data, start_date_str, end_date_str, device_name, local_timezone):
    records = []
    ...
    for data_point, ts in points:
        ...
        records.append({'measurement': 'Skin Temperature Variation', 'time': ts, 'tags': {'Device': device_name}, 'fields': fields})
        inserted_count += 1
    if inserted_count:
        logger.info('Recorded Skin Temperature Variation for date %s to %s (Google mode): %s points', start_date_str, end_date_str, inserted_count)
    else:
        logger.warning('No Skin Temp records found for date %s to %s in Google mode', start_date_str, end_date_str)
    return [HealthPoint.from_record(record) for record in records]


def map_oxygen(data, start_date_str, end_date_str, device_name, local_timezone):
    records = []
    ...
    for data_point, ts in points:
        ...
        if value is None:
            continue
        records.append({'measurement': 'SPO2_Intraday', 'time': ts, 'tags': {'Device': device_name}, 'fields': {'value': float(value)}})
        inserted_count += 1
    if inserted_count:
        logger.info('Recorded SPO2 intraday for date %s to %s (Google mode): %s points', start_date_str, end_date_str, inserted_count)
    else:
        logger.warning('No SPO2 intraday records found for date %s to %s in Google mode', start_date_str, end_date_str)
    return [HealthPoint.from_record(record) for record in records]
