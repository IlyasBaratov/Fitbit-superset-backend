...
def map_body(data, start_date_str, end_date_str, device_name, local_timezone):
    records = []
    height_samples = []
    ...
    for data_point in height_response.get('dataPoints', []) if isinstance(height_response, dict) else []:
        ...
        records.append({'measurement': 'height', 'time': height_time, 'fields': height_fields})
    ...
    for data_point, ts in points:
        ...
        records.append({'measurement': 'weight', 'time': weight_time, 'fields': weight_fields})
        inserted_count += 1
        ...
        if bmi is not None:
            records.append({'measurement': 'bmi', 'time': weight_time, 'fields': {'value': bmi, 'weightKg': weight_fields.get('weightKg'), 'heightMeters': height_meters, 'isCalculated': True}})
            inserted_count += 1
    if inserted_count:
        logger.info('Recorded weight and BMI for date %s to %s (Google mode): %s points', start_date_str, end_date_str, inserted_count)
    else:
        logger.warning('No Weight/BMI records found for date %s to %s in Google mode', start_date_str, end_date_str)
