...
def map_spo2(
    ...
    if points:
            ...
            records.append(
                {
                    ...
                    "fields": {"avg": avg_value, "max": max_value, "min": min_value},
                }
            )
            inserted_count += 1
        logger.info(
            "Recorded Avg SPO2 for date %s to %s (Google mode): %s points",
            start_date_str,
            end_date_str,
            inserted_count,
        )
        ...
        logger.warning(
            "No daily oxygen saturation records found for date %s to %s in Google mode",
            start_date_str,
            end_date_str,
        )
