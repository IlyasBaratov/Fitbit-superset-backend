...
from app.providers.google_health import (
    vitals,
    body,
    sleep,
    activity,
...
class GoogleHealthProvider:
    ...
    def fetch_intraday(self, date: str, measurements=INTRADAY) -> list[HealthPoint]:
        data = {
            kind: self._available(
                kind, self.client.get_google_datapoints_for_date, kind, date
            )
            or []
            for kind in ("heart-rate", "steps")
        }
        return intraday.map_intraday(
            data, date, measurements, self.device_name, self.timezone
        )
    ...
    def fetch_daily_group(self, group: str, start: str, end: str) -> list[HealthPoint]:
        ...
        if group == "100d":
            return sleep.map_sleep(
                {"sleep": self._range("sleep", start, end)},
                start,
                end,
                self.device_name,
                self.timezone,
        ...
        if group == "none":
            return spo2.map_spo2(
                {
                    "daily-oxygen-saturation": self._range(
                        "daily-oxygen-saturation", start, end
                    )
                },
                start,
                end,
    ...
    def _vitals(self, start, end):
        ...
        height = (
            self._available(
                "height",
                self.client.request_google_data_points_list,
                "height",
                params={"pageSize": 100},
            )
            or {}
        ...
        records.extend(
            body.map_body(
                {"height": height, "weight": self._range("weight", start, end)},
                start,
                end,
                self.device_name,
                self.timezone,
    ...
    def _activity(self, start, end):
        records = activity.map_resting(
            {
                "daily-resting-heart-rate": self._range(
                    "daily-resting-heart-rate", start, end
                )
            },
            start,
            end,
        ...
        for kinds, mapper in (
            ...
            for kind in kinds:
                ...
                while day <= last:
                    ...
                    data[kind][day.strftime("%Y-%m-%d")] = (
                        self._available(
                            kind,
                            self.client.request_google_data_points_daily_rollup,
                            kind,
                            payload,
                        )
                        or {}
    ...
    def fetch_workouts(self, end: str | None = None) -> list[HealthPoint]:
        ...
        return workouts.map_workouts(
            {"exercise": data}, self.device_name, self.timezone
        )
    ...
    def fetch_device_metadata(self) -> list[HealthPoint]:
        ...
        fields = sanitize_fields(
            {
                "deviceName": self._metadata.get("deviceName"),
                "deviceModel": self._metadata.get("deviceModel"),
                "timezone": self.timezone.zone,
                "firmwareVersion": self._metadata.get("firmwareVersion"),
                "connectionStatus": self._metadata.get("connectionStatus"),
            }
        )
        return [
            HealthPoint(
                "Device Metadata",
                self._metadata.get("_observationTime")
