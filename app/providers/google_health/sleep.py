...
from app.domain.normalization import sanitize_fields, sleep_efficiency, sleep_stage, stable_resource_id
logger = logging.getLogger(__name__)



def map_sleep(data, start_date_str, end_date_str, device_name, local_timezone):
    records = []
    ...
    for data_point, ts in points:
        ...
        records.append({'measurement': 'Sleep Summary', 'time': start_time_str, 'tags': {'Device': device_name, 'isMainSleep': is_main_sleep}, 'fields': sanitize_fields({'SleepSessionId': sleep_session_id, 'efficiency': efficiency, 'minutesAfterWakeup': minutes_after_wakeup, 'minutesAsleep': minutes_asleep, 'minutesToFallAsleep': minutes_to_fall, 'minutesInBed': minutes_in_period, 'minutesAwake': minutes_awake, 'minutesLight': minutes_light, 'minutesREM': minutes_rem, 'minutesDeep': minutes_deep, 'startTime': start_time_str, 'endTime': session_end_time_str})})
        inserted_count += 1
        for stage in sleep.get('stages', []):
            ...
            if stage_end_time_str:
                try:
                    ...
                    pass
            records.append({'measurement': 'Sleep Levels', 'time': stage_ts, 'tags': {'Device': device_name, 'isMainSleep': is_main_sleep}, 'fields': sanitize_fields({'SleepSessionId': sleep_session_id, 'level': level, 'stageName': stage_name, 'duration_seconds': duration_secs})})
        if session_end_time_str:
            try:
                ...
                records.append({'measurement': 'Sleep Levels', 'time': wake_ts, 'tags': {'Device': device_name, 'isMainSleep': is_main_sleep}, 'fields': {'SleepSessionId': sleep_session_id, 'level': 3, 'stageName': 'awake'}})
            except ValueError:
                pass
    if inserted_count:
        logger.info('Recorded Sleep data for date %s to %s (Google mode): %s sessions', start_date_str, end_date_str, inserted_count)
    else:
        logger.warning('No Sleep records found for date %s to %s in Google mode', start_date_str, end_date_str)
