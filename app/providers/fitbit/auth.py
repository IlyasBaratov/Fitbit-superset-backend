...
class FitbitTokenManager(FileTokenManager):
    ...
    def _request_refresh(self):
        cfg = self.settings
        if not cfg.client_id or not cfg.client_secret or cfg.client_id == "your_application_client_ID":
        ...
        return self.session.post(f"{cfg.fitbit_api_base_url}/oauth2/token",
            headers={"Authorization": "Basic " + credentials, "Content-Type": "application/x-www-form-urlencoded"},
            data={"grant_type": "refresh_token", "refresh_token": self._refresh_token}, timeout=cfg.request_timeout_seconds)
