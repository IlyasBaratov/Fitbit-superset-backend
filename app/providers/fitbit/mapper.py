...
def map_intraday(
    ...
    date_str: str,
    measurement_list: Sequence[tuple[str, str, str]],
    device_name: str,
    local_timezone: BaseTzInfo,
) -> list[HealthPoint]:
    records = []
    for measurement in measurement_list:
        data = payloads.get("intraday:" + measurement[0], {})[
            "activities-" + measurement[0] + "-intraday"
        ]["dataset"]
        if data != None:
            for value in data:
...
def map_spo2(
    ...
    if data_list != None:
        ...
        logger.info(
            "Recorded Avg SPO2 for date " + start_date_str + " to " + end_date_str
        )
    else:
        logger.error(
...
def map_workouts(
    ...
    if recent_activities_data != None:
        for activity in recent_activities_data["activities"]:
            fields = {
                "ActivityId": str(activity["logId"])
                if activity.get("logId") is not None
                else None,
            ...
            starttime = datetime.fromisoformat(
                utc_timestamp(activity["startTime"], local_timezone.zone)
            )
            utc_time = starttime.astimezone(pytz.utc).isoformat()
            if activity.get("duration") is not None:
                fields["endTime"] = (
                    starttime + timedelta(milliseconds=float(activity["duration"]))
                ).isoformat()
            ...
            ActivityID = (
                fields.get("ActivityId") or utc_time + "-" + extracted_activity_name
            )
            ...
            records.append(
                {
                    ...
                    "time": utc_time,
                    "tags": {"ActivityName": extracted_activity_name},
                    "fields": fields,
                }
            )
        ...
        logger.error(
            "Fetching 50 recent activities failed : before date " + end_date_str
        )
    return [HealthPoint.from_record(record) for record in records]
