...
def map_activity(
    ...
    for activity_type in activity_minutes_list:
        activity_minutes_data_list = payloads.get(
            "activity_series:" + activity_type, {}
    ...
    for activity_type in activity_others_list:
        ...
        activity_others_data_list = payloads.get(
            "activity_series:" + activity_type, {}
        ).get("activities-tracker-" + activity_type)
    ...
    if HR_zones_data_list != None:
            ...
            records.append(
                {
                    ...
                    "fields": {
                        ...
                        "Fat Burn": data["value"]["heartRateZones"][1].get(
                            "minutes", 0
                        ),
        ...
        logger.info(
            "Recorded RHR and HR zones for date "
            + start_date_str
            + " to "
            + end_date_str
        )
        ...
        logger.error(
            "Recording failed : RHR and HR zones for date "
            + start_date_str
            + " to "
            + end_date_str
        )
    HR_zone_minutes_list = payloads.get("active_zone_minutes", {}).get(
    ...
    if HR_zone_minutes_list != None:
        ...
        logger.info(
            "Recorded HR zone minutes for date "
            + start_date_str
            + " to "
            + end_date_str
        )
        ...
        logger.error(
            "Recording failed : HR zone minutes for date "
            + start_date_str
            + " to "
            + end_date_str
        )
