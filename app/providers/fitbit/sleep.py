...
def map_sleep(payloads, start_date_str, end_date_str, device_name, local_timezone):
    records = []
    ...
    if sleep_data != None:
        for record in sleep_data:
            ...
            try:
                ...
                minutesDeep = 0
            records.append({'measurement': 'Sleep Summary', 'time': utc_time, 'tags': {'Device': device_name, 'isMainSleep': is_main_sleep}, 'fields': sanitize_fields({'SleepSessionId': sleep_session_id, 'efficiency': record['efficiency'], 'minutesAfterWakeup': record['minutesAfterWakeup'], 'minutesAsleep': record['minutesAsleep'], 'minutesToFallAsleep': record['minutesToFallAsleep'], 'minutesInBed': record['timeInBed'], 'minutesAwake': record['minutesAwake'], 'minutesLight': minutesLight, 'minutesREM': minutesREM, 'minutesDeep': minutesDeep, 'startTime': record.get('startTime'), 'endTime': record.get('endTime')})})
            for stage in record['levels']['data']:
                ...
                records.append({'measurement': 'Sleep Levels', 'time': utc_time, 'tags': {'Device': device_name, 'isMainSleep': is_main_sleep}, 'fields': {'SleepSessionId': sleep_session_id, 'level': level, 'stageName': stage_name, 'duration_seconds': stage.get('seconds')}})
            ...
            records.append({'measurement': 'Sleep Levels', 'time': utc_wake_time, 'tags': {'Device': device_name, 'isMainSleep': is_main_sleep}, 'fields': {'SleepSessionId': sleep_session_id, 'level': 3, 'stageName': 'awake'}})
