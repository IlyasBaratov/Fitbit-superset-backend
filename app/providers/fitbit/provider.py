...
class FitbitProvider:
    ...
    def fetch_intraday(self, date: str, measurements=INTRADAY) -> list[HealthPoint]:
        payloads = {}
        for kind, _, resolution in measurements:
            payloads["intraday:" + kind] = self._available(
                kind, self.client.intraday, kind, date, resolution
            ) or {"activities-" + kind + "-intraday": {"dataset": []}}
        return mapper.map_intraday(
            payloads, date, measurements, self.device_name, self.timezone
        )
    ...
    def fetch_daily_group(self, group: str, start: str, end: str) -> list[HealthPoint]:
        if group == "30d":
            ...
            for method, mapping in (
                ("hrv", vitals.map_hrv),
                ("breathing", vitals.map_breathing),
                ("skin_temperature", vitals.map_temperature),
                ("spo2_intraday", vitals.map_oxygen),
                ("weight", vitals.map_body),
            ):
                result = self._available(
                    method, getattr(self.client, method), start, end
                )
                points.extend(
                    mapping(
                        {method: result or ([] if method == "spo2_intraday" else {})},
                        start,
                        end,
                        self.device_name,
                        self.timezone,
        ...
        if group == "100d":
            ...
            return sleep.map_sleep(
                {"sleep": result}, start, end, self.device_name, self.timezone
            )
        if group == "365d":
            payloads = {}
            for kind in (
                ...
                payloads["activity_series:" + kind] = (
                    self._available(kind, self.client.activity_series, kind, start, end)
                    or {}
                )
            for method in ("heart_summary", "active_zone_minutes"):
                payloads[method] = (
                    self._available(method, getattr(self.client, method), start, end)
                    or {}
                )
            return activity.map_activity(
                payloads, start, end, self.device_name, self.timezone
            )
        if group == "none":
            result = self._available("spo2", self.client.spo2, start, end) or []
            return mapper.map_spo2(
                {"spo2": result}, start, end, self.device_name, self.timezone
            )
    ...
    def fetch_workouts(self, end: str | None = None) -> list[HealthPoint]:
        ...
        before = (datetime.strptime(end, "%Y-%m-%d") + timedelta(days=1)).strftime(
            "%Y-%m-%d"
        )
        data = self._available(
            "activities",
            self.client.activities,
            params={"beforeDate": before, "sort": "desc", "limit": 50, "offset": 0},
        ) or {"activities": []}
        points = mapper.map_workouts(
            {"activities": data}, end, self.device_name, self.timezone
        )
        count = 0
        for record in data.get("activities", []):
            if record.get("hasGps") and record.get("tcxLink") and count <= 10:
                ...
                activity_id = (
                    str(record["logId"])
                    if record.get("logId") is not None
                    else utc_timestamp(record["startTime"], self.timezone.zone)
                    + "-"
                    + name
                )
                try:
                    points.extend(
                        tcx.map_tcx(response.text, activity_id, name, self.timezone)
                    )
    ...
    def fetch_battery(self) -> list[HealthPoint]:
        ...
        if (
            not devices
            or devices[0].get("batteryLevel") is None
            or not devices[0].get("lastSyncTime")
        ):
            return []
        return [
            HealthPoint(
                "DeviceBatteryLevel",
                utc_timestamp(devices[0]["lastSyncTime"], self.timezone.zone),
                {"value": float(devices[0]["batteryLevel"])},
            )
        ]
    ...
    def fetch_device_metadata(self) -> list[HealthPoint]:
        ...
        fields = sanitize_fields(
            {
                "deviceName": device.get("deviceVersion") or self.device_name,
        ...
        return [
            HealthPoint(
                "Device Metadata",
                device.get("lastSyncTime") or datetime.now(timezone.utc).isoformat(),
                fields,
            )
        ]
