...
class InfluxHealthRepository:
    def __init__(self, settings, common_tags, timezone, client=None):
        self.settings, self.common_tags, self.timezone = (
            settings,
            dict(common_tags),
            timezone,
        )
    ...
    def _create_client(self):
        ...
        return InfluxDBClient3(
            host=f"http://{cfg.influxdb_host}:{cfg.influxdb_port}",
            token=cfg.influxdb_v3_access_token,
            database=cfg.influxdb_database,
        )
    ...
    def _detect_field_types(self):
        for measurement in ("RestingHR", "Total Steps"):
            try:
                ...
                ).get_points(measurement=measurement)
                value = next(
                    (row for row in rows if row.get("fieldKey") == "value"), None
                )
            except Exception:
                logger.warning("Could not inspect field types for %s", measurement)
                continue
            if value and value.get("fieldType") in {"float", "integer"}:
                self.field_types[measurement]["value"] = (
                    float if value["fieldType"] == "float" else int
                )
    ...
    def write(self, points: list[HealthPoint]) -> bool:
        ...
        prepared = prepare_points(
            records, self.common_tags, self.timezone, self.field_types
        )
        ...
        if not prepared or self.settings.dry_run_mode:
            logger.info(
                "No write: %d validated points, dry_run=%s",
                len(prepared),
                self.settings.dry_run_mode,
            )
            return False
        try:
