...
- Queries use at most 190 days and 20,000 returned rows per measurement, failing rather than silently truncating. Intraday series are aggregated hourly in InfluxDB first. Each database request has a 10-second timeout; each Gemini call has a 45-second timeout and no transport retries. Invalid model output is retried once. Prepared model payloads are limited to 120 KB and outputs to 50,000 characters.
